...
def connection():
  ...
  if (size.isdigit() == False or size == ''):
    ...
    return
  if any(map(str.isdigit, shrift))==True or shrift == '' or nachert == '' or color == '' or any(map(str.isdigit, nachert))==True or any(map(str.isdigit, color))==True:
    warning_label.config(text="Ошибка")
    return

  rez = entry_shrift.get()+ ' '+ entry_size.get()+ ' '+ entry_nachert.get()+ ' '+ entry_color.get()
...
label_comboBox = tk.Label(text='Пользователь: ',font='Arial',) 
label_comboBox.grid(row=4, column=0)
comboBox = ttk.Combobox(values=mas)
comboBox.grid(row = 4, column=1)


shrift = tk.Label(text='Название шрифта: ',font='Arial',) 
shrift.grid(row=0, column=0)
size = tk.Label(text='Размер шрифта: ',font='Arial',) 
size.grid(row=1, column=0)
nachert = tk.Label(text='Начертание: ',font='Arial',) #italic - naklon
nachert.grid(row=2, column=0)
label_color = tk.Label(text='Цвет: ',font='Arial',) 
label_color.grid(row=3, column=0)

comboBox.bind("<<ComboboxSelected>>", callbackFunc)

entry_shrift = tk.Entry(font='Arial',)
entry_shrift.grid(row=0,column=1)
entry_size = tk.Entry(font='Arial',)
entry_size.grid(row=1,column=1)
entry_nachert = tk.Entry(font='Arial',)
entry_nachert.grid(row=2,column=1)
entry_color = tk.Entry(font='Arial',)
